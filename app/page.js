  ...
  return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 p-4 md:p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white rounded-2xl p-6 shadow-lg"><p className="text-gray-500 text-sm">Totalt invitert</p><p className="text-3xl font-bold text-indigo-600">719</p></div>
            <div className="bg-white rounded-2xl p-6 shadow-lg"><p className="text-gray-500 text-sm">Respondenter</p><p className="text-3xl font-bold text-indigo-600">202</p></div>
            <div className="bg-white rounded-2xl p-6 shadow-lg"><p className="text-gray-500 text-sm">Opererte</p><p className="text-3xl font-bold text-indigo-600">285</p></div>
            <div className="bg-white rounded-2xl p-6 shadow-lg"><p className="text-gray-500 text-sm">Periode</p><p className="text-3xl font-bold text-indigo-600">4 aar</p></div>
    </div>
          <div className="flex flex-wrap gap-2 mb-8 bg-gray-100 p-2 rounded-2xl">
  {['overview','health','symptoms','employment'].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-3 rounded-xl font-medium ${activeTab === tab ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600'}`}>
...
{activeTab === 'overview' && (
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl p-6 shadow-lg">
                 ...
                 <ResponsiveContainer width="100%" height={300}>
                    <PieChart><Pie data={treatmentData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="value" label={({percent}) => `${percent.toFixed(1)}%`}>{treatmentData.map((e,i) => <Cell key={i} fill={COLORS[i]} />)}</Pie><Tooltip/></PieChart>
  </ResponsiveContainer>
  </div>
              <div className="bg-white rounded-2xl p-6 shadow-lg">
                  <h3 className="text-lg font-semibold mb-4">Helsescorer</h3>
                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={compositeData} layout="vertical"><CartesianGrid/><XAxis type="number" domain={[0,100]}/><YAxis dataKey="name" type="category" width={100}/><Tooltip/><Legend/><Bar dataKey="consultation" name="Konsultasjon" fill={CHART_COLORS.consultation}/><Bar dataKey="operated" name="Operert" fill={CHART_COLORS.operated}/></BarChart>
  </ResponsiveContainer>
  </div>
  </div>
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                  <div className="bg-white/10 rounded-xl p-4"><p className="text-3xl font-bold">+15.2</p><p className="text-sm">Hoyere fysisk helsescore</p></div>
                <div className="bg-white/10 rounded-xl p-4"><p className="text-3xl font-bold">+15.3</p><p className="text-sm">Hoyere mental helsescore</p></div>
                <div className="bg-white/10 rounded-xl p-4"><p className="text-3xl font-bold">-24%</p><p className="text-sm">Lavere symptombelastning</p></div>
  </div>
  </div>
  </div>
        )}
{activeTab === 'health' && (
            <div className="space'use client';
  import React, { useState } from 'react';
 import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar, PieChart, Pie, Cell } from 'recharts';
...
export default function Dashboard() {
    ...
    const rand12Data = [
      ...
      { domain: 'SF', fullName: 'Sosial funksjon', consultation: 60.0, operated: 67.5, diff: 7.5 },
      { domain: 'RE', fullName: 'Emosjonell rollefunksjon', consultation: 26.9, operated: 51.9, diff: 25.1 },
      { domain: 'MH', fullName: 'Mental helse
